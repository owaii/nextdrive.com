...
function CarCard({ car_id, with_buttons, selectable, selected, onSelect, smoothScroll }: CarCardProps) {
  ...
  return (
    <section
      ...
      className={`
        w-80 xl:w-120 flex flex-col rounded-3xl overflow-hidden shadow-md 
    ...
    >
      ...
      <section className="w-full border border-black border-t-0 rounded-b-3xl flex flex-col">
        <div className="w-full flex items-center justify-center pt-5 px-5">
          <span className="text-3xl xl:text-4xl font-bold">{car.title}</span>
        </div>
        <div className="w-full flex items-center justify-center">
          <span className="xl:text-lg italic text-center">{car.gearbox}</span>
        </div>
...
export default function CarHero({ with_buttons, selectable, selected, onSelectCar, smoothScroll }: CarHeroProps) {
  ...
  return (
    <section className="Main w-full flex items-center justify-center mx-auto pt-20 gap-15 bg-(--WarmWhite) flex-wrap">
      {carList.map((car, i) => (
        <CarCard
          key={i}
          car_id={i}
