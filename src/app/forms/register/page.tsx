    ...
    <div className="LoginFormBackground w-screen h-screen flex items-center justify-center">
      <div className="w-[90%] sm:w-[70%] md:w-[50%] lg:w-[40%] xl:w-[35%] 2xl:w-[30%] h-auto min-h-[400px] flex flex-col p-6 py-10 rounded-2xl transition-all cursor-pointer bg-gradient-to-b from-[#f7fcff] via-[#e9f7fc] to-[#d9f1fa] border border-cyan-200 border-t-white shadow-[0_0_34px_2px_rgba(66,68,90,0.15)] hover:shadow-[0_0_40px_4px_rgba(66,68,90,0.25)] transition-shadow duration-300">
        ...
        <div className="InputFieldsCon w-full flex flex-col px-4 sm:px-10 gap-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative w-full">
              ...
              <input type="text" id="name" className=" border border-(--bg-secondary) text-(--txt-primary) text-sm rounded-lg focus:ring-(--bg-special) focus:border-(--bg-special) block w-full pl-10 p-2.5" placeholder="Imię" maxLength={20}/>
            </div>
            <div className="relative w-full">
              <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-(--txt-primary)" fill="currentColor" viewBox="0 0 20 20"><path d="M10 10a4 4 0 1 0-4-4 4 4 0 0 0 4 4Zm0 2c-3.31 0-6 2.69-6 6h12c0-3.31-2.69-6-6-6Z" /></svg>
              </div>
              <input type="text" id="surname" className=" border border-(--bg-secondary) text-(--txt-primary) text-sm rounded-lg focus:ring-(--bg-special) focus:border-(--bg-special) block w-full pl-10 p-2.5" placeholder="Nazwisko" maxLength={20}/>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <input type="tel" id="phone" value={phone} onChange={handlePhoneChange} inputMode="numeric" className=" border border-(--bg-secondary) text-(--txt-primary) text-sm rounded-lg focus:ring-(--bg-special) focus:border-(--bg-special) block w-full pl-10 p-2.5" placeholder="Numer telefonu" />
          </div>
          ...
          <div className="relative">
            ...
            <input type="email" id="email" className=" border border-(--bg-secondary) text-(--txt-primary) text-sm rounded-lg focus:ring-(--bg-special) focus:border-(--bg-special) block w-full pl-10 p-2.5" placeholder="Email" />
          </div>
          ...
          <div className="relative">
            ...
            <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} className=" border border-(--bg-secondary) text-(--txt-primary) text-sm rounded-lg focus:ring-(--bg-special) focus:border-(--bg-special) block w-full pl-10 p-2.5" placeholder="Hasło" />
          </div>
        ...
        <div className="SignInButton w-full flex items-center justify-center py-4">
          <button className="LoginButton w-full p-2 rounded-xl text-(--txt-secondary) bg-foreground hover:opacity-90 transition cursor-pointer">
            Zarejestruj się
          </button>
        </div>
