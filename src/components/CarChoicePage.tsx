    ...
    <section className="w-full flex-col">
      ...
      <section className="w-full flex flex-col gap-1 mt-2">
        <div className="w-full flex items-center justify-start">
          <span className="text-l text-txt-black font-bold tracking-wide">{title}</span>
        </div>
        <div className="w-full flex items-center justify-start">
          <span className="text-xs text-txt-black font-extralight tracking-wide">{gearbox}</span>
        </div>
      </section>
    </section>
  );
  ...
  return (
    <section className="w-full py-10 flex flex-col bg-brand-white">
      ...
      <section className={clsx(
        ...
        {
          cars.map((car) => (
            <ShowCar key={car.id} title={car.title} gearbox={car.gearbox} image_src={car.image_src} />
          ))
        }
      </section>
    </section>
